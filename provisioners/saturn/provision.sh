...
scp -q "${ssh_opts[@]}" provisioners/saturn/install.sh $TARGET:/home/nixos/install.sh
ssh -q "${ssh_opts[@]}" $TARGET 'chmod +x ~/install.sh'

# Copy the configuration files templates
scp -q "${ssh_opts[@]}" provisioners/saturn/hardware-configuration.tpl.nix $TARGET:/home/nixos/hardware-configuration.tpl.nix
scp -q "${ssh_opts[@]}" provisioners/saturn/configuration.tpl.nix $TARGET:/home/nixos/configuration.tpl.nix
