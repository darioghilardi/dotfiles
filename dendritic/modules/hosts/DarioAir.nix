# Phase-A migration of DarioAir: reproduces the snowfall-generated darwin
# toplevel on raw flake-parts, reusing the original module files verbatim
# (under ../../reused). Goal: byte-identical store path vs the root flake.
{
  inputs,
  lib,
  ...
  ...
  extendedLib = inputs.nixpkgs.lib.extend (final: _prev: {
  ...
  # vs snowfall's direct path imports, so descending here cancels that out. This
  # is Phase-A scaffolding; it goes away when features become native aspect files
  # in Phase B.)
  collectModules = dir:
  ...
  # it (nix-darwin for darwin modules, home-manager for home modules — each
  # passes the correct context-specific config/pkgs).
  wrap = modPath: {
    config,
    pkgs,
    options,
  ...
  flake.darwinConfigurations.${hostName} = inputs.darwin.lib.darwinSystem {
    modules =
      [
        ...
        (wrap ../../reused/DarioAir-system.nix)
        {
          ...
          home-manager.users.dario.imports = [(wrap ../../reused/DarioAir-home.nix)];
        }
      ]
